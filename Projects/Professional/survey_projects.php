...
// Most recent Field Data QC raw data path per project, keyed by project_id.
// Warmed by one batched request (mirrors taskCache above) so the "Field Data"
// link button can open the crew's actual OneDrive/network path instead of
// guessing one from the Survey folder link.
let projectRawDataPathCache = {};
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Load projects and warm the task/raw-data-path caches in parallel.
    // If the path cache resolves after projects are already rendered,
    // re-render so the Field Data links pick up the resolved paths.
    warmTaskCache();
    warmRawDataPathCache().then(() => {
        if (allProjects.length) updateProjectsDisplay();
    });
    loadProjects();
...
// Warms projectRawDataPathCache with each project's most recent Field Data QC
// raw data path in a single request. get_sessions with no project_id returns
// every session ordered by project_id, collection_date DESC, session_id DESC —
// so the first session seen per project here is the most recent one.
function warmRawDataPathCache() {
    return fetch('../../Models/php/field_data_qc_api.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
        },
    ...
    .then(response => response.json())
    .then(data => {
        projectRawDataPathCache = {};
        if (data.success) {
            (data.sessions || []).forEach(s => {
                if (s.raw_data_path && !projectRawDataPathCache[s.project_id]) {
                    projectRawDataPathCache[s.project_id] = s.raw_data_path;
                }
            });
        }
        return projectRawDataPathCache;
    })
    .catch(error => {
        console.error('Error warming raw data path cache:', error);
        projectRawDataPathCache = {};
        return projectRawDataPathCache;
    });
}
...
function createProjectLinks(project) {
    ...
    // Field Data: prefer the raw data path actually logged in Field Data QC
    // (often a OneDrive/SharePoint link the crew was given) over guessing a
    // local \Downloads path off the Survey folder link.
    const surveyPath = project.surveyFolderLink;
    const loggedRawPath = projectRawDataPathCache[project.projectId] || '';
    const fieldDataLink = loggedRawPath
        ? { path: loggedRawPath, isUrl: isLikelyUrl(loggedRawPath), icon: 'fas fa-download', label: 'Field Data', color: '#3b82f6' }
        : (surveyPath ? { path: `${surveyPath}\\\\Downloads`, isUrl: false, icon: 'fas fa-download', label: 'Field Data', color: '#3b82f6' } : null);

    const controlLink = surveyPath
        ? { path: `${surveyPath}\\\\Control`, isUrl: false, icon: 'fas fa-map-pin', label: 'Control', color: '#10b981' }
        : null;

    const autoGeneratedLinks = [fieldDataLink, controlLink].filter(Boolean);
    ...
    // Build auto-generated links — a logged Field Data path that's a cloud
    // link opens directly; everything else goes through the file:// UNC path
    const autoLinks = autoGeneratedLinks
        .map(link => `
            <a href="${link.isUrl ? link.path : `file:///${link.path.replace(/N:\\/g, uncPath)}`}"
               ${link.isUrl ? 'target="_blank" rel="noopener noreferrer"' : ''}
               class="link-button" style="border-color: ${link.color};">
                <i class="${link.isUrl ? 'fas fa-cloud' : link.icon}" style="color: ${link.color};"></i>
                ${link.label}
...
// Field Data QC raw data paths are often cloud folder links (OneDrive/
// SharePoint) rather than local/network paths — detect that so the Field
// Data button can open it directly instead of going through file:// + UNC.
