...
function createProjectLinks(project) {
    ...
    // Control points live in the app's own database, not a guessed folder —
    // link straight to this project's Control Points page.
    const controlLink = `
        <a href="./control_points.php?project_id=${encodeURIComponent(project.projectId)}" class="link-button" style="border-color: #10b981;">
            <i class="fas fa-crosshairs" style="color: #10b981;"></i>
            Control
        </a>
    `;

    // Raw field data lives wherever the most recent QC session's raw data path
    // says (recorded per-session, not a fixed "\Downloads" subfolder) — copy it
    // straight to clipboard instead of guessing a file:// link that may be wrong.
    const fieldDataCopyBtn = `
        <button type="button" class="link-button" style="border-color: #3b82f6; cursor: pointer; font: inherit;"
                onclick="copyFieldDataPath('${project.projectId}')" title="Copy raw field data path from the most recent QC session">
    ...
    return dbLinks + controlLink + fieldDataCopyBtn + qcLink;
}

// Copies the raw field data path from the most recent QC session that has one
// recorded — mirrors the "Copy raw data path" button in the qc-path-row div
// on field_data_qc.php, but reachable straight from the dashboard.
async function copyFieldDataPath(projectId) {
    try {
        const formData = new FormData();
        formData.append('action', 'get_sessions');
        formData.append('project_id', projectId);
        const response = await fetch('../../Models/php/field_data_qc_api.php', { method: 'POST', body: formData });
        const data = await response.json();
        const sessions = data.success ? (data.sessions || []) : [];
        // Sessions come back newest first; skip any that haven't had a raw
        // data path recorded yet rather than failing on the very latest one.
        const withPath = sessions.find(s => s.raw_data_path);

        if (!withPath) {
            showToast('No raw field data path recorded for this project yet', 'error');
            return;
        }
        copyFolderPath(withPath.raw_data_path, 'Field data path');
    } catch (error) {
        console.error('Error copying field data path:', error);
        showToast('Network error copying field data path', 'error');
    }
}
